...
export default function EditUserForm(props) {
  ...
  const onDropeBanner = useCallback((acceptedFile) => {
  ...
  const {
    ...
    noKeyboard: true,
    multiple: false,
    onDrop: onDropeBanner
  });

  const onDropeAvatar = useCallback((acceptedFile) => {
    const file = acceptedFile[0];
    ...
    <div className="edit-user-form">
      ...
      <Form onSubmit={onSubmit}>
        ...
        <Form.Group>
          <DatePicker
            locale="es"
            value={new Date(formData.fechaNacimiento)}
            clearIcon={null}
            onChange={(value) =>
